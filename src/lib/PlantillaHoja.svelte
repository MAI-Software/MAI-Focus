<script lang="ts">
  ...
  import type { Energia, Forma, Plantilla } from './tipos';
  ...
  // svelte-ignore state_referenced_locally
  let color = $state(plantilla?.color ?? COLORES_CAT[estado.plantillas.length % COLORES_CAT.length].valor);
  // svelte-ignore state_referenced_locally
  let forma = $state<Forma>(plantilla?.forma ?? 'circulo');
  // svelte-ignore state_referenced_locally
...
<Hoja titulo={editando ? 'Plantilla' : 'Nueva plantilla'} {onCerrar}>
  ...
  <SelectorMarca bind:color bind:forma ayuda="Los bloques creados con esta plantilla nacen con esta marca." />
